...
```{r}
file_list <- list.files(cities_files) %>%
  squash_string()
...
```{r}
file_list <- list.files(cities_summary_files) %>%
  squash_string()
head(file_list)
...
```{r}
file_list <- list.files(states_files) %>%
  squash_string()
 ...
 **States yearly files**
```{r}
file_list <- list.files(states_summary_files) %>%
  squash_string()
...
**Check that cities names in cities_df with errors **
```{r}
(master_table <- read_xls("RELATORIO_DTB_BRASIL_MUNICIPIO.xls") %>%
    mutate(key =
             squash_string(make_place_key(
               Nome_Município, Nome_UF
             )))
)
